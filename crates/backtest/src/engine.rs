...
use crate::{
    ...
    execution_client::{BacktestExecutionClient, SubmissionRecord},
    result::{
...
impl BacktestEngine {
    ...
    /// Returns a snapshot of all submissions generated by this engine's execution clients.
    #[must_use]
    pub fn submission_records(&self) -> Vec<SubmissionRecord> {
        self.exec_clients
            .iter()
            .flat_map(BacktestExecutionClient::submission_records)
            .collect()
    }
    ...
    pub fn reset(&mut self) -> anyhow::Result<()> {
        ...
        for client in &self.exec_clients {
            client.clear_submission_records();
        }
...
mod tests {
    ...
    #[derive(Debug)]
    struct LifecycleSubmissionStrategy {
        core: StrategyCore,
        instrument_id: InstrumentId,
    ...
    impl DataActor for LifecycleSubmissionStrategy {
    ...
    struct QuoteSubmissionStrategy {
        core: StrategyCore,
        instrument_id: InstrumentId,
        submissions_per_quote: Vec<usize>,
        quote_count: usize,
        client_order_ids: Rc<RefCell<Vec<ClientOrderId>>>,
    }

    impl QuoteSubmissionStrategy {
        fn new(
            strategy_id: StrategyId,
            instrument_id: InstrumentId,
            submissions_per_quote: Vec<usize>,
    ...
    impl DataActor for QuoteSubmissionStrategy {
        fn on_start(&mut self) -> anyhow::Result<()> {
    ...
    nautilus_strategy!(QuoteSubmissionStrategy);

    #[derive(Debug)]
    struct EmulationReleaseStrategy {
        core: StrategyCore,
        instrument_id: InstrumentId,
        submitted: bool,
        client_order_id: Rc<Cell<Option<ClientOrderId>>>,
    }

    impl EmulationReleaseStrategy {
        fn new(
            instrument_id: InstrumentId,
            client_order_id: Rc<Cell<Option<ClientOrderId>>>,
        ) -> Self {
            Self {
                core: StrategyCore::new(StrategyConfig {
                    strategy_id: Some(StrategyId::from("EMULATION-RELEASE-001")),
                    ..Default::default()
                }),
                instrument_id,
                submitted: false,
    ...
    impl DataActor for EmulationReleaseStrategy {
        fn on_start(&mut self) -> anyhow::Result<()> {
            self.subscribe_quotes(self.instrument_id, None, None);
            Ok(())
        }
        ...
        fn on_quote(&mut self, _quote: &QuoteTick) -> anyhow::Result<()> {
            ...
            self.client_order_id.set(Some(order.client_order_id()));
            self.submit_order(order, None, None, None)
        }
    }

    nautilus_strategy!(EmulationReleaseStrategy);
    ...
    fn quote_data(instrument_id: InstrumentId, bid: &str, ask: &str, ts: u64) -> Data {
    ...
    fn test_submission_records_survive_run_finalization(crypto_perpetual_ethusdt: CryptoPerpetual) {
        ...
        engine
            .add_strategy(LifecycleSubmissionStrategy::new(
                StrategyId::from("SUBMIT-ON-START-001"),
                instrument_id,
                true,
                false,
            ))
        ...
        let records_before_end = engine.submission_records();
        assert_eq!(records_before_end.len(), 1);
        engine.end().unwrap();
        assert_eq!(engine.submission_records(), records_before_end);
    }

    #[rstest]
    fn test_on_stop_submission_is_recorded(crypto_perpetual_ethusdt: CryptoPerpetual) {
        let mut engine = create_engine();
        let instrument = InstrumentAny::CryptoPerpetual(crypto_perpetual_ethusdt);
        let instrument_id = instrument.id();
        engine.add_instrument(&instrument).unwrap();
    ...
    #[rstest]
    fn test_failed_cache_application_preserves_reused_id_submission_lineage(
        ...
        let instrument_id = instrument.id();
        engine.add_instrument(&instrument).unwrap();
        let client_order_id = ClientOrderId::from("O-BUFFERED-REUSE");
        ...
        engine
            ...
            .cache
            .borrow_mut()
            .add_order(first.clone(), None, Some(ClientId::from("BINANCE")), false)
            .unwrap();
        execute_submission(&engine, &first, UUID4::new(), UnixNanos::default());
        engine
            .kernel
            .cache
            .borrow_mut()
            .add_order(second.clone(), None, Some(ClientId::from("BINANCE")), true)
            .unwrap();
        execute_submission(&engine, &second, UUID4::new(), UnixNanos::default());
        ...
        let event_count_before = engine.kernel.exec_engine.borrow().event_count();
        engine.drain_exec_client_events();
        assert_eq!(
            engine.kernel.exec_engine.borrow().event_count() - event_count_before,
            2
        );

        {
            ...
            assert_eq!(cached.event_count(), 2);
        }
        assert_eq!(engine.submission_records(), records);
    }

    #[rstest]
    fn test_submission_records_reset_and_engine_isolation(
        crypto_perpetual_ethusdt: CryptoPerpetual,
    ) {
        let mut engine = create_engine();
        ...
        engine
            ...
            .unwrap();
        execute_submission(&engine, &order, UUID4::new(), UnixNanos::default());
        engine.drain_exec_client_events();
        assert_eq!(engine.submission_records().len(), 1);

        engine.reset().unwrap();
        assert!(engine.submission_records().is_empty());
        engine
            .run(
                Some(UnixNanos::from(0)),
                Some(UnixNanos::from(1)),
        ...
        let another_order = OrderTestBuilder::new(OrderType::Market)
            .trader_id(engine.trader_id())
            .instrument_id(instrument_id)
            .client_order_id(ClientOrderId::from("O-AFTER-RESET"))
            .quantity(Quantity::from("1.000"))
            .build();
        engine
            .kernel
    ...
    #[rstest]
    fn test_streaming_clear_data_keeps_submission_records(
        crypto_perpetual_ethusdt: CryptoPerpetual,
    ) {
        let mut engine = create_engine();
        let instrument = InstrumentAny::CryptoPerpetual(crypto_perpetual_ethusdt);
        let instrument_id = instrument.id();
        engine.add_instrument(&instrument).unwrap();
        let client_order_ids = Rc::new(RefCell::new(Vec::new()));
        engine
            .add_strategy(QuoteSubmissionStrategy::new(
                StrategyId::from("STREAMING-SUBMISSIONS-001"),
                instrument_id,
                vec![1, 1],
                Rc::clone(&client_order_ids),
            ))
            .unwrap();
        engine
            .add_data(
    ...
    #[rstest]
    fn test_risk_denial_does_not_create_submission_record(
        crypto_perpetual_ethusdt: CryptoPerpetual,
    ) {
        let config = BacktestEngineConfig {
            risk_engine: Some(RiskEngineConfig {
                max_order_submit: RateLimit::new(1, DurationNanos::from_secs(1)),
                ..Default::default()
            }),
            run_analysis: false,
            ..Default::default()
        ...
        engine
            .add_strategy(QuoteSubmissionStrategy::new(
                StrategyId::from("RATE-LIMITED-SUBMISSIONS-001"),
                instrument_id,
                vec![2, 1],
                Rc::clone(&client_order_ids),
            ))
        ...
        engine
            .add_data(
                vec![
                    quote_data(instrument_id, "1000.00", "1001.00", 1_000_000_000),
                    quote_data(instrument_id, "1001.00", "1002.00", 3_000_000_000),
                ],
                None,
                true,
                true,
        ...
        let ids = client_order_ids.borrow();
        assert_eq!(ids.len(), 3);
        let records = engine.submission_records();
        assert_eq!(records.len(), 2);
        assert_eq!(records[0].client_order_id, ids[0]);
        assert_eq!(records[1].client_order_id, ids[2]);
        assert!(
            records
                .iter()
                .all(|record| record.client_order_id != ids[1])
        );
        assert_eq!(
            engine
                .kernel
    ...
    #[rstest]
    fn test_emulation_release_records_exactly_once(crypto_perpetual_ethusdt: CryptoPerpetual) {
        let mut engine = create_engine();
        let instrument = InstrumentAny::CryptoPerpetual(crypto_perpetual_ethusdt);
        let instrument_id = instrument.id();
        engine.add_instrument(&instrument).unwrap();
        let client_order_id = Rc::new(Cell::new(None));
        engine
            .add_strategy(EmulationReleaseStrategy::new(
                instrument_id,
                Rc::clone(&client_order_id),
            ))
            .unwrap();
        engine
            .add_data(
        ...
        assert!(engine.submission_records().is_empty());
        let client_order_id = client_order_id.get().unwrap();
        let expected_init_id = engine
            .kernel
            .cache
            .borrow()
            ...
            .unwrap()
            .init_id();
        let expected_command_id = engine
            .kernel
            .order_emulator
            .get_emulator()
            .get_submit_order_commands()[&client_order_id]
            .command_id;
        ...
        let released_init_id = engine
            ...
            .unwrap()
            .init_id();
        assert_ne!(released_init_id, expected_init_id);
        assert_eq!(records[0].init_id, released_init_id);
        assert_eq!(records[0].command_id, expected_command_id);
        assert_eq!(records[0].ts_event, UnixNanos::from(2_000_000_000));
        assert_eq!(records[0].ts_init, UnixNanos::from(2_000_000_000));
        engine.end().unwrap();
    }
